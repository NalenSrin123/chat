...
class ChatController extends Controller {
 ...
 public function send(Request $request,Conversation $conversation):RedirectResponse|\Illuminate\Http\JsonResponse { $this->allowed($conversation); $data=$request->validate(['message'=>'nullable|string|max:5000','attachment'=>'nullable|image|mimes:jpg,jpeg,png,gif,webp|max:5120']); if(!$request->filled('message')&&!$request->hasFile('attachment')) return back()->withErrors(['message'=>'Write a message or choose an image.']); $payload=['sender_id'=>auth()->id(),'message'=>$data['message']??'','type'=>'text']; if($request->hasFile('attachment')){$payload['type']='image';$payload['file_path']=$request->file('attachment')->store('chat','public');} $message=$conversation->messages()->create($payload); broadcast(new MessageSent($message))->toOthers(); $conversation->touch(); if($request->expectsJson()) return response()->json(['message'=>$message->load('sender')]); return back(); }
